...
usage() {
    ...
    echo "    ./runtests.sh tests/unit[/<test-suite>]"
...
run_static_tests() {
    ...
    codespell -q4 -L keyserver \
      -S "*.tar,*.xz,*.zip,*.bz2,*.7z,*.gz,*.deb,*.rpm,*.snap,*.gpg,*.pyc,*.png,*.ico,*.jar,changelog,.git,.hg,.mypy_cache,.tox,.venv,_build,buck-out,__pycache__,build,dist,.vscode,parts,stage,prime,./.direnv"

    echo "Running shellcheck"
o "Running shellcheck inside spread yaml"
